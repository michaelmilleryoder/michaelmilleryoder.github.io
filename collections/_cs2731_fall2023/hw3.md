...
3. documentation that probability distributions for all language models are valid (sum to 1). Specifically, where $$w$$ is a character (random variable) and $$c$$ is the prior context of that character, this means that $$\sum_{i}^{} P(w_i \vert c) = 1, \forall c$$. It is okay for the sum to be not exactly 1, but it should be close to 1 (>0.97 and <1.03 is fine). Adding special start and end characters to every line when you read in the file may help probabilities sum to 1. If the prior context $$c$$ has never been seen before by the model (and thus the sum of the probability distribution over all characters given that context is 0), it can simply be excluded from this calculation.
